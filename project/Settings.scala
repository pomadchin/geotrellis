...
object Settings {
  ...
  lazy val raster = Seq(
    ...
    libraryDependencies ++= Seq(
      pureconfig,
      jts,
      catsCore(scalaVersion.value),
      spire,
      squants,
      monocleCore(scalaVersion.value),
      monocleMacro(scalaVersion.value),
      scalaXml,
      scalaURI(scalaVersion.value),
      scalatest % Test,
      scalacheck % Test
    ),
  ...
  lazy val spark = Seq(
    ...
    libraryDependencies ++= Seq(
      ...
      chronoscala,
      catsCore(scalaVersion.value),
      catsEffect(scalaVersion.value),
      fs2Core(scalaVersion.value),
  ...
  lazy val `spark-pipeline` = Seq(
    ...
    libraryDependencies ++= Seq(
      circeCore(scalaVersion.value),
      circeGeneric(scalaVersion.value),
      circeGenericExtras(scalaVersion.value),
      circeParser(scalaVersion.value),
      sparkCore % Provided,
      sparkSQL % Test,
      scalatest % Test
    ),
